...
class BookAPITests(APITestCase):
    ...
    def test_detail_book_public_ok(self):
        ...
        response = self.client.get(url)
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["id"], self.tfa.id)
    ...
    def test_create_book_authenticated_201(self):
        ...
        self.assertEqual(response.data["title"], "The Interpreter")
        self.assertEqual(response.data["publication_year"], 1965)
        self.assertEqual(response.data["author"], self.soyinka.id)
    ...
    def test_update_book_requires_auth(self):
        ...
        response = self.client.patch(url, {"publication_year": 1959}, format="json")
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
    ...
    def test_update_book_authenticated_200(self):
        ...
        response = self.client.patch(url, {"publication_year": 1959}, format="json")
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["publication_year"], 1959)
    ...
    def test_filter_publication_year_exact(self):
        url = reverse("book-list")
        response = self.client.get(url, {"publication_year": 1960})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data["count"], 1)
        self.assertEqual(response.data["results"][0]["title"], "No Longer at Ease")
    ...
    def test_filter_title_icontains(self):
        url = reverse("book-list")
        response = self.client.get(url, {"title": "fall"})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertTrue(any(b["title"] == "Things Fall Apart" for b in response.data["results"]))

    def test_filter_year_range(self):
        url = reverse("book-list")
        response = self.client.get(url, {"min_year": 1950, "max_year": 1970})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        titles = [b["title"] for b in response.data["results"]]
        self.assertIn("Things Fall Apart", titles)
        self.assertIn("No Longer at Ease", titles)
